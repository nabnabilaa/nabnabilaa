...
main{width:880px;margin:32px auto;padding:24px;border:1px solid #d0d7de;border-radius:6px}
...
for (const scheme of ['light', 'dark']) {
  const page = await browser.newPage({ viewport: { width: 1000, height: 900 }, colorScheme: scheme, deviceScaleFactor: 1 });
  ...
  await page.screenshot({ path: join(out, `preview-${scheme}.png`), fullPage: true });
  await page.setViewportSize({ width: 1000, height: 900 });
  // the first viewport, mid-animation, to check the entrance
  await page.reload(); await page.waitForTimeout(450);
  await page.screenshot({ path: join(out, `preview-${scheme}-t450.png`) });
  await page.close();
}
await browser.close();
console.log('done');
